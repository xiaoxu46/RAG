...
本项目最初是一个**基础 RAG 对话系统**，在获得 200+ Star 后，我们做了一次重要转型：

| | 阶段一（base-rag 分支） | 阶段二（master 分支） |
|--|-----------------------|---------------------|
| **定位** | 纯 RAG 对话服务，开箱即用 | 智能笔记助手，以 RAG 为核心的 KM 工具 |
| **能力** | 文档上传 → 向量检索 → AI 问答 | 笔记管理 + RAG + 间隔重复 + AI 写作 |
| **适合谁** | 想快速集成 RAG 能力的开发者 | 需要完整知识管理方案的团队/个人 |

**RAG 始终是整个系统的核心引擎。** 基础 RAG 代码已永久保留在 `base-rag` 分支，与 master 同步维护，如果你只需要纯 RAG 服务，`git checkout base-rag` 即可开箱使用。

感谢所有 Star 用户的长期支持，你们的认可让我们有动力做得更好 🙏
